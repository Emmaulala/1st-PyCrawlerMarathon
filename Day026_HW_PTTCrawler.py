...
# 範例目標網址: https://www.ptt.cc/bbs/HatePolitics/M.1577003035.A.F39.html
class PttcrawlerSpider(scrapy.Spider): #S大寫
    name = 'PTTCrawler'  #執行程式時要輸入scrapy crawl PTTCrawler
    allowed_domains = ['www.ptt.cc']
    start_urls = ['https://www.ptt.cc/bbs/HatePolitics/M.1577003035.A.F39.html']
    cookies = {'over18': '1'}

    def start_requests(self): #self表實體物件本身
        for url in self.start_urls:
            yield scrapy.Request(url=url, callback=self.parse, cookies=self.cookies) #Request經過包裝後被scrapy管理，scrapy中發送request的用法: yield scrapy.Request('url')

    def parse(self, response):  #作為上面的callback
        ...
        try:
            ...
            ip = re.search('[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*', ip).group()  #.group() 返回完整匹配的ip (整串ip)
